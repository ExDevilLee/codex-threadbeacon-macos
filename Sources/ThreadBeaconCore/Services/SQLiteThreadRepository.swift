...
public struct SQLiteThreadRepository: Sendable {
    ...
    public func loadRecentSubagentCandidates(
        parentIDs: [String],
        updatedAfter: Date
        ...
        let cutoffMilliseconds = Int64(updatedAfter.timeIntervalSince1970 * 1_000)
        guard sqlite3_bind_int64(
            statement,
            Int32(parentIDs.count + 1),
            cutoffMilliseconds
        ) == SQLITE_OK else {
            throw SQLiteThreadRepositoryError.database(databaseMessage(database))
        }

        var recordsByParent: [String: [SubagentActivityCandidate]] = [:]
        while true {
            switch sqlite3_step(statement) {
            case SQLITE_ROW:
                guard
                ...
                let parentID = String(cString: parentIDText)
                let record = SubagentActivityCandidate(
                    id: String(cString: idText),
                    parentID: parentID,
                    rolloutPath: String(cString: rolloutText),
                    updatedAt: Date(
