...
- 使用事件顶层 `timestamp`；当 payload 中存在合法的 `completed_at` 时，使用两者中较晚的时间。
- 不保存 turn ID、reason、消息正文或其他 payload 内容。
- 缺少 reason、reason 不匹配或时间无效的事件不改变任务状态。
...
- 排序位置：`warning` 之后、`running` 之前
- 状态持续时间：沿用 `statusChangedAt`

灰色与停止符号共同表达“已停止但不是故障”。中断任务值得被看见，因此排在运行中之前，但不抢占
错误、需要操作和服务异常。
...
- malformed 或其他 reason 的 abort 不改变状态。
- 中断事件不会进入声音通知和自动恢复链路。
