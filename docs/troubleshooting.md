...
徽标使用`活跃数/总数`，例如`2/27`。分子只统计最近 rollout 中已确认正在运行的直接
Subagent；分母表示历史上创建过的直接 Subagent 总数。展开主任务后会读取直接子任务详情；
当前不显示第二层及更深任务树，也不聚合父子 Token。
