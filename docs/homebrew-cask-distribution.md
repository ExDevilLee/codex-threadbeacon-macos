...
> 实现状态：MVP 已完成。项目自有 Tap 已公开，下载、校验、隔离安装和卸载均已验证；真实
> 跨版本升级与维护者回滚等待下一个 Release 验证。
...
1. `brew style Casks/threadbeacon.rb`。
2. `brew audit --cask --strict ExDevilLee/tap/threadbeacon`。
3. `brew fetch --cask ExDevilLee/tap/threadbeacon`，SHA-256 校验通过。
4. 使用隔离 `--appdir` 安装，不覆盖本机 `/Applications` 中的 App。
5. 校验 App 版本 `0.1.4`、Bundle ID、`arm64 + x86_64` 架构和现阶段 ad-hoc 签名。
6. `brew uninstall --cask threadbeacon` 正确移除隔离安装。
7. GitHub Actions 在干净的 macOS Runner 重复执行 style、audit、下载和隔离安装／卸载。
