# 服务异常监控

## 结论

ThreadBeacon 可以在不接管 Codex 任务、不读取会话正文的前提下，从本机只读日志中识别
当前可见主任务的结构化 HTTP 4xx/5xx 异常、自动重试与最终失败、重新连接重试耗尽后的最终断流，
以及明确记录的所选模型容量错误。

这个能力补充了 rollout 状态推导。自动恢复默认关闭；用户可按 HTTP 400、HTTP 429、HTTP 503、
其他终止型 HTTP 错误、模型容量异常和连接中断配置规则与提示词。HTTP 503 默认关闭，活跃重试不触发。
规则启用后只通过用户授权的 macOS Accessibility 控制 Codex App，外部 `codex exec resume` 继续禁用。
启动时已有的历史异常只建立基线、不发送，同一 episode 每次运行只处理一次。授权等待目前也没有
可靠数据源。

## 真实验证记录

2026-07-20 使用一个真实 Codex 主任务进行现场验证：ThreadBeacon 正确把结构化 HTTP 400
关联到可见主任务，并显示红色 `Service failed` 状态、`HTTP 400` 详情和异常持续时间。
该验证同时确认 400 状态不会被 rollout 中可能存在的通用完成事件覆盖。原始会话 ID、任务
标题和日志正文不进入仓库或公开文档；截图仅作为本地验证证据保留。

2026-07-22 对真实连接中断样本完成只读验证：同一 turn 先记录重新连接 `5/5`，随后记录精确的
最终断流 `Turn error`。ThreadBeacon 只有在这两项证据同时存在时才标记
`failed + streamDisconnected + 5/5`；样本会话 ID、任务标题、URL 和完整日志正文未写入仓库。

2026-07-23 发现并修复一类真实 429 日志格式遗漏：Codex 在重试耗尽时会写入
`Turn error: exceeded retry limit, last status: 429 Too Many Requests`，状态码前使用冒号，
而不是结构化请求记录中的 `status=429`。旧解析器因此把该任务错误回退成“刚完成”；现在同时接受
`status=429`、`status 429` 和 `status: 429`，并沿用同一 `HTTP 429` 失败、异常音和自动恢复规则。
该真实样本确认了会话 ID、SQLite 日志链路和 UI 状态之间的回归测试覆盖。

## 数据源

App 以 SQLite read-only 模式打开：

```text
~/.codex/logs_2.sqlite
```

查询限定当前可见任务 ID，并只允许以下 target：

| Target | 提取字段 | 用途 |
| --- | --- | --- |
| `codex_http_client::default_client` | turn ID、HTTP 状态、时间 | 旧版 Codex：异常与恢复 |
| `codex_http_client::client` | turn ID、HTTP 状态、时间 | 兼容新版 Codex，识别异常与同 turn 恢复 |
| `codex_core::responses_retry` | turn ID、重试次数与上限、时间 | 显示自动重试进度 |
| `codex_core::session::turn` | turn ID、最终 HTTP 状态、精确最终断流、模型容量错误、时间 | 识别终止失败 |

`codex_http_client::transport` 被明确排除，因为它可能包含完整请求上下文。查询也不选择
feedback tags、工具输出或其他日志 target。

## 状态规则

| 证据 | 展示 | 提示音 |
| --- | --- | --- |
| 出现结构化 HTTP 4xx/5xx（不含 503） | 红色 `error`，显示 HTTP 状态 | 每个 turn episode 一次异常音 |
| 429/503 自动重试中 | 黄色 `warning`，显示状态和 `n/limit` | 每个 episode 一次异常音 |
| 同 turn 后续出现 200 | 清除 warning，回到 rollout 推导状态 | 不补播声音 |
| 重试耗尽并出现 `Turn error` | 红色 `error` | 同 episode 已提醒则不重复播放 |
| 重连 `5/5` 后最终断流 | 红色 `error`，显示“连接中断”和 `5/5` | 已提醒则不重复播放 |
| 出现明确的所选模型容量 `Turn error` | 红色 `error`，显示“所选模型容量已满” | 每个 turn episode 一次异常音 |
| 更晚出现新的 `task_started` | 清除旧 incident | 新 turn 若再次异常，视为新 episode |

服务 incident 优先于 rollout 的 `task_complete`。这是因为失败 turn 也可能写入通用
`task_complete`；若不覆盖，会把真实失败误报为完成并播放错误的完成音。

恢复提示来自对应异常规则，不读取或拼接会话正文；当前不会启动外部 CLI，不改变异常状态，也不
阻塞后续刷新。未授权时记录未发送；已授权时通过任务 ID、rename 标题、输入框安全检查和 rollout
结果确认完成发送。

### Codex App 内可见的恢复

外部 `codex exec resume` 走的是独立 CLI 执行通道，不能保证 Codex App 当前窗口和侧边栏同步显示
消息，因此当前版本不调用它。若要达到“Codex App 会话中可见并继续执行”的效果，需要通过 macOS
Accessibility 控制 Codex App 的输入框和发送动作；这要求用户在系统设置中单独授权 ThreadBeacon。
没有该授权时，App 只记录未发送，不伪装成已完成 Codex App 内恢复。

方案 A 的研究记录见 [`accessibility-recovery-poc.md`](accessibility-recovery-poc.md)。

## 隐私边界

- 原始白名单日志行仅在一次刷新解析期间存在，不进入 `ThreadSnapshot`、界面或本地偏好。
- App 只保留 turn episode ID、异常类型、HTTP 状态、重试进度、阶段和时间。
- UI 不显示供应商 URL、request ID、完整请求、日志正文或会话内容。
- 日志数据库读取失败时，App 降级为不显示 incident，不影响现有任务列表。

## 已知限制

- `logs_2.sqlite` 是 Codex 内部滚动日志，不是稳定公开 API，字段、target 或日志形状可能
  随版本变化。
- 日志轮转后，较早的异常证据可能消失。
- 当前只接受白名单 target 中明确的 HTTP 4xx/5xx 结构化形态（例如 `status=429`、`status 429`
  和 `status: 429`）、精确的最终断流和模型容量
  `Turn error`，不把
  2xx、静默、超时或宽泛正文关键词推断为服务异常。日志轮转后，
  过去的异常仍可能无法追溯；现场验证只证明验证时刻的结构化日志链路，不代表历史日志永久可查。
- 该能力不能识别授权请求、用户输入等待或所有失败类型。

## 验证

自动测试覆盖：

- 503 重试耗尽转为 `error`。
- 429 活跃重试转为 `warning`。
- 结构化 400 请求完成记录立即转为 `error`。
- 同 turn 200 清除 warning。
- 新旧 `codex_http_client` target 的同 turn 200 都能清除 warning。
- 明确的所选模型容量错误转为 `error`，且不伪造 HTTP 状态。
- 单独的重连 `5/5` 仍为 `warning`；同 turn 后续出现精确最终断流时才转为
  `streamDisconnected + error`，且不伪造 HTTP 状态。
- `transport` target 即使包含 429 也被忽略。
- incident 覆盖误导性的 `task_complete`。
- 同 episode 只产生一次异常提示音。
- 所有终止型 HTTP episode、模型容量异常和最终连接中断只产生一次结构化恢复候选，启动时历史 episode 不发送；
  HTTP 503 由默认关闭的独立规则过滤，活跃重试不产生候选。

运行：

```bash
./script/test.sh
./script/build_and_run.sh --verify
./script/probe.sh
```
