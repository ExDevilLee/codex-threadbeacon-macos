...
`~/.codex/logs_2.sqlite` 的白名单结构化日志可只读识别 HTTP 400 Bad Request，以及
HTTP 429/503 的自动重试和最终失败，现已作为独立数据源实现；它仍不能识别授权等待。详见
...
当前仓库的 `Tools/AppServerProbe/send-message.mjs` 已提供受保护的最小验证工具：默认只
执行 `thread/resume` Dry Run；只有同时传入 `--send --confirm-send` 才会调用
`turn/start`，并通过 `input: [{ type: "text", text: ... }]` 发送一条消息。

该能力证明的是 Codex CLI 实验性 app-server 协议具备写入接口，不证明它能接入 Codex
Desktop 当前已运行的 app-server。ThreadBeacon 主 App 当前不调用独立 app-server 或
`codex exec resume` 进行自动恢复；外部 CLI 发送仅保留为开发研究 POC，不能把消息注入
并同步到已经运行的 Codex App 进程。要实现可见恢复，需继续验证 Accessibility 方案 A。
...
以上限制针对独立 app-server。ThreadBeacon 当前改由白名单日志证据识别 400/429/503，不把
该实现描述成 app-server 能力，也不据此扩展到授权或其他错误。

### 不采用的回退

- 不扫描会话正文猜测 `400`、`429`、`503` 或授权状态；HTTP 状态只接受白名单 target 中
  明确的结构化日志形状。
