...
Desktop 当前已运行的 app-server。ThreadBeacon 主 App 的 400 自动续做 MVP 采用更稳定的
`codex exec resume` CLI 入口，而不是直接启动独立 app-server；它同样可能使用独立运行时，
因此不宣称能把消息注入已经运行的 Codex App 进程。
