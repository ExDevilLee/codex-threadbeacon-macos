...
该结果证明同名和列表顺序不再决定目标，但在当时还不能启用无人值守自动恢复。实测中，ThreadBeacon
...
- 用户主动点击与无人值守模式分开建模。手动测试允许 Codex 浮动窗口仍保持 active，但继续执行草稿
  检查；无人值守模式在 Codex 保持前台且唯一输入框为空时允许按任务 ID deep link 切换，存在草稿、
  多个输入框或输入框不可读时直接停止；
...
- 无人值守前台保护最初由纯策略单测覆盖，随后已接入自动异常恢复主链路；2026-07-23 修复了
  Codex 前台但目标任务未选中时的误判，空输入框现在可以安全切换并继续完成标题二次确认。
...
2026-07-22 已接入无人值守发送后的原前台 App 安全恢复：发送前记录原 App 与 Codex PID，发送返回
后只有当前前台仍是同一 Codex 进程且原 App 仍运行时才恢复。如果用户期间切换到第三个 App，或
原 App 已退出、是 Codex、身份不可用，则静默跳过。该逻辑不作用于 Debug 手工发送和双击打开任务；
Core 策略、完整测试与 AppKit 构建已通过，真实异常自动恢复场景仍需持续观察。
...
- 自动恢复总开关默认关闭，不在用户未明确开启时申请或使用 Accessibility 权限。
- 发送前必须完成目标会话匹配；匹配失败时只记录失败，不发送。
- 不使用固定屏幕坐标作为唯一定位依据。
- 不读取或保存会话正文；只注入对应异常规则中用户可见、可编辑的提示词。
- deep link 前检查当前输入框数量和草稿；冲突时停止，不覆盖或迁移草稿。
- 无人值守操作在 Codex 保持前台时，仅当唯一可读输入框为空才允许按任务 ID 切换；存在草稿、多个
  输入框或输入框不可读时停止。用户主动测试仍需明确确认。
- 发送后必须通过 rollout 的新 `user_message` / `task_started` 做结果确认。
- 连续失败、窗口不可见、ID 导航失败、rename 标题不匹配或 AX 树结构变化时自动停止。
- 保留 CLI 发送 POC 作为开发研究工具，但不作为主 App 的自动恢复路径。

## 与当前实现的关系

当前自动恢复不会调用独立的 `codex exec resume`。Release 设置提供总开关和 HTTP 400、HTTP 429、
HTTP 503、其他终止型 HTTP 错误、模型容量异常的独立规则与提示词；HTTP 503 默认关闭。规则启用但
未授权时记录“未发送”，已授权时才通过已验证的目标定位、输入和 rollout 回读链路发送。

手工只读诊断、输入框验证、任务 ID 和测试发送入口只保留在 Debug 构建，Release 不渲染这些控件。
无人值守发送后会按
[`自动恢复后恢复原前台 App`](auto-recovery-foreground-restoration-design.md) 的安全策略尝试恢复焦点。
