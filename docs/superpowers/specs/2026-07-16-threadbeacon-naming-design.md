...
项目正式品牌确定为 `ThreadBeacon`，面向用户和 App Store 的完整名称为
`ThreadBeacon for Codex`。

- 英文产品名：`ThreadBeacon for Codex`
- App 显示名：`ThreadBeacon`
- 中文功能描述：`macOS 上的 Codex 任务状态窗`
- 英文副标题：`A glanceable Codex task monitor for macOS`
- 中文昵称：“Codex 红绿灯”，只作为功能类比，不再作为正式产品名
- GitHub repository slug：`codex-threadbeacon-macos`
...
`ThreadBeacon` 保持为品牌主体，`for Codex` 明确兼容对象并保留 App Store 和 GitHub
搜索可发现性。该结构比把 `Codex` 放在名称开头更不容易形成官方附属产品的观感，
也比 `CodexThreadBeacon` 连写更易读。
...
- README 标题和开头描述改为 `ThreadBeacon for Codex`。
...
- 私有仓库从 `codex-traffic-light` 重命名为 `codex-threadbeacon-macos`。
- 重命名前确认目标 slug 当时仍可用。
...
未来如果验证 Windows 或外接显示端，优先使用独立平台仓库，例如
`codex-threadbeacon-windows`。各仓库独立维护代码、Issues、Releases 和平台
ROADMAP，并在 README 的 `Related projects` 中互相引用。只有在两个以上平台实现
产生了真实重复逻辑后，才评估共享 core；现在不创建空的总入口仓库或共享库。
...
7. GitHub 私有仓库名称为 `codex-threadbeacon-macos`，description、README 链接和
   本地 `origin` 与之保持一致。
