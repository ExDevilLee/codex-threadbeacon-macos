...
Feature 6A 已于 2026-07-17 按推荐的行内展开方案完成：

- 点击主任务的 Subagent 数量标记，展开或收起直接子任务。
- 默认以 `Agent 别名 ｜ 标题` 显示名称，并显示状态、最近活动和自身累计 Token；别名为空
  或与标题相同时不重复显示。
- info 详情显示昵称、角色、模型、Reasoning effort 和 Token 明细。
- 只为已展开且当前可见的父任务批量读取直接 Subagent；收起状态不解析子任务 rollout。
- 展开状态只保留在本次 App 生命周期，不读取会话正文、不显示更深任务树、不聚合父子
  Token。

真实 UI 已用包含 3 个直接 Subagent 的任务验证。Feature 6B 压缩可观测性仍保持独立候选，
没有并入本次实现。
...
Feature 6A 已按“主任务行内展开”方案实现。压缩历史信息可以与 6A 共用详情数据模型，
但不要把 Hook 或 app-server 实时接入并入当前功能；Feature 6B 继续保留为独立 POC，
避免放大当前 MVP 的数据兼容风险。
