...
- App 只在本地持久化窗口是否钉在最前面、刷新间隔、最大显示任务数、提示音开关与选择，
  以及最多 256 个通知事件 ID。事件 ID 只包含主任务 ID、完成时间或异常 episode ID 与
  事件类别，不包含任务标题、HTTP 状态、消息正文、reasoning、命令、URL、request ID
  或文件内容。
...
子命令，但当前没有用户可触发入口。音频播放失败不会影响状态读取。
