...
- 数据源健康报告也只保存在当前进程内存中，包含稳定状态类别、Rollout 成功／失败数量和
  ...
  文件读取范围。
- App 不上传数据，不启动网络服务，也不直接写入或修改 Codex SQLite。
- About 窗口展示运行中 App 的版本和公开项目链接，不读取 Codex 数据。只有用户主动点击
  GitHub、版本记录、隐私、License 或支持项目链接时，App 才把对应公开 URL 交给 macOS
  默认浏览器；App 不在后台加载、重试或缓存这些页面，也不保存点击记录。
...
授权等待或所有错误类型。底层归档恢复 POC 依赖本机 Codex CLI 和受支持的 `unarchive`
子命令，但当前没有用户可触发入口。音频播放失败不会影响状态读取。登录时启动要求系统
