...
- `~/.codex/logs_2.sqlite` 中当前可见主任务的少量结构化日志。查询仅允许
  `codex_http_client::default_client`、`codex_http_client::client`、
  `codex_core::responses_retry` 和 `codex_core::session::turn` 四个 target，并只提取
  turn ID、HTTP 400/429/503、重试次数、明确的模型容量错误类别和最终失败时间。两个
  `codex_http_client` target 分别兼容旧版和新版 Codex 的相同结构化完成事件。
...
- 压缩历史统计默认只读 rollout。只有用户在 Settings 明确启用实时压缩状态时，App 才会备份并
  结构化合并 `~/.codex/hooks.json` 中由 ThreadBeacon 管理的 `PreCompact` 和 `PostCompact`
  处理器，同时安装本地 Hook Helper。已有 Hook 保持不变，Codex 的 Hook trust 流程不会被绕过；
  Settings 可移除 ThreadBeacon 条目、Helper 和活动标记。Helper 不保存 Hook 输入中的会话正文、
  摘要、Reasoning、工作目录或 transcript 路径。完整边界见
...
- 自动恢复熔断状态写入
  `~/Library/Application Support/ThreadBeacon/auto-recovery-circuit-breaker.json`，
  仅保存任务 ID、异常类型、连续尝试次数、最近 episode ID 和时间；不保存任务标题、提示词、
  会话正文或错误原文。
  正常完成会清除该任务的计数，用户也可在 Settings 中解除指定任务和异常类型的熔断。
