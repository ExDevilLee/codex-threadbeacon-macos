...
- 数据源健康报告也只保存在当前进程内存中，包含稳定状态类别、Rollout 成功／失败数量和
  ...
  文件读取范围。
- App 不上传 Codex 数据，不启动网络服务，也不直接写入或修改 Codex SQLite。检测到新的主任务
  终止型 HTTP 400、429 或模型容量异常时，App 当前只记录固定恢复提示为“未发送：需要 macOS
  Accessibility 授权”；不会调用外部 Codex CLI，也不读取或拼接会话正文。HTTP 503 不触发恢复
  提示，启动时历史异常不补发，同一 episode 每次运行只记录一次。
...
- 当前版本不使用通讯录、位置、相机或麦克风权限。Settings 的“自动恢复”页会读取当前 App 的
  ...
  显式打开系统设置。授权后，只有用户点击“验证 Codex 只读访问”时，App 才遍历 Codex App 的
  Accessibility 结构，并只在内存中统计窗口、输入框和访问节点数量；不读取、显示或保存元素标题、
  值与会话内容，也不切换任务。只读诊断通过后，用户可单独点击当前输入框验证：App 只在输入框
  唯一、为空或仅包含已验证占位值时，短暂写入固定提示词，回读后立即清空并再次确认；检测到草稿
  时拒绝覆盖。该验证不查找发送按钮、不模拟回车，不保存输入框值，也不接入自动恢复主链路。
  用户也可输入目标任务 ID 手动验证任务切换：App 在内存中读取该 ID 对应的 rename 标题，只有
  本地标题映射和 Codex 任务行均唯一时才执行 `AXPress`，随后通过 Codex 顶部标题栏和唯一输入框
  再次确认身份。任务 ID 和标题不会因此写入日志或持久化，且该操作不写入或发送消息。自动发送
  仍然关闭。未来若启用“在 Codex App 内注入恢复消息”，仍需补齐发送确认和撤销授权说明。
- App 只在本地持久化窗口是否钉在最前面、主窗口显示器标识与 frame、刷新间隔、最大
  显示任务数、提示音开关与选择、自定义提示音文件路径，以及最多 256 个通知事件 ID。
...
- 自动恢复日志写入 `~/Library/Application Support/ThreadBeacon/auto-recovery-log.json`，最多保留
  200 条，包含会话 ID、异常 episode ID、异常类型、固定提示词、时间和脱敏后的发送结果；不会写入任务标题、
