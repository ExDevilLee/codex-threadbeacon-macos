...
只读数据源。
检测到新的主任务终止型 HTTP 4xx/5xx（HTTP 503 除外）或模型容量异常时，App 会记录异常并记录恢复消息为“未发送”；
当前不会调用外部 Codex CLI 恢复。只有用户明确授予 macOS Accessibility 权限、并完成方案 A 的
Codex App 输入框定位验证后，才会考虑把固定提示词“刚才中断了，请继续未完成的任务”注入 Codex App。
...
- 新的主任务终止型 HTTP 4xx/5xx（HTTP 503 除外）和模型容量异常 episode 会记录一次固定恢复提示，
  当前版本不会自动发送。
...
- 只读诊断通过后，可手动验证当前 Codex 输入框：仅当输入框唯一且没有草稿时，短暂写入固定
  提示词，回读后立即清空并再次确认。该验证不查找发送按钮、不模拟回车，也不会接入自动恢复。
- 还可输入目标任务 ID 手动验证切换：App 先确认当前 Codex 只有一个输入框且没有草稿，再使用
  `codex://threads/<thread-id>` 按 ID 打开目标，并以顶部 rename 名称二次确认。检测到草稿或多个
  输入框，或无法确认输入框为空时，会在 deep link 前停止，不写入或发送消息。
- 用户明确确认后，测试入口可发送固定恢复提示；结果只以指定 ID 对应 rollout 中严格匹配的新消息
  和 `task_started` 为准。同名任务交换列表位置前后均已验证只命中指定 ID。
- 无人值守模式的预检策略会在 Codex 保持前台时停止，但该模式尚未接入自动异常恢复主链路。
...
授权并点击验证后使用；只读诊断仅返回结构计数，输入框验证只短暂写入固定提示词并立即清空。
...
输入框内容不会持久化。该测试入口未接入无人值守自动恢复。
App 启动后只向 `api.github.com` 请求公开
Release 元数据以检查更新；请求不包含
Codex 数据、本机路径、用户设置或设备标识。
当前公开 UI 不直接修改 Codex SQLite；HTTP 400 等异常自动处理当前只记录未发送的固定恢复提示，不调用
