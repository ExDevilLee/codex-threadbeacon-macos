...
- 还可输入目标任务 ID 手动验证切换：只有任务 ID 对应的 rename 标题、Codex 任务行和切换后的
  顶部标题身份均唯一时才视为成功；该验证不写入输入框，也不发送消息。
- 当前只能切换已在 Codex AX 树中渲染的任务行；较旧或未加载任务会失败关闭，因此该 POC 尚未
  接入无人值守自动恢复。
