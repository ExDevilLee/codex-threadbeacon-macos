...
- 标题栏齿轮按钮打开原生 macOS Settings 窗口；`通用`页配置刷新间隔、最大显示任务数和
  登录时启动，`提示音`页管理完成与 429/503 服务异常声音。登录时启动通过 macOS 官方
  `SMAppService.mainApp` 管理，开关直接反映系统状态；如果系统要求批准，App 会保留开启
  状态并提供登录项设置入口。两类提示音可分别关闭、从六种内置声音中选择并试听。设置会
...
- 登录时启动依赖 macOS 识别当前 App bundle。当前 `dist/ThreadBeacon.app` 使用 ad hoc
  签名并从开发目录运行，实测系统返回 `notFound`，因此开关会禁用。正式启用前需要把稳定
  构建放在固定位置并使用有效 Developer ID 签名，再验证注册、系统审批、重启登录和注销。
