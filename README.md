...
容量异常分别启用规则并配置提示词。启用后只有终止型异常会触发，503 仍默认关闭。发送必须由用户
...
2. 首次启动如果被 macOS 拦截，请在 Finder 中按住 Control 点击 App，选择“打开”。
...
尚未公证，首次打开边界与下面的 GitHub Release 安装方式相同。
...
- 自动恢复总开关默认关闭；HTTP 400、HTTP 429、其他终止型 HTTP 错误和模型容量规则预置开启，
  HTTP 503 规则预置关闭。每类提示词均可编辑并恢复默认；内置默认值跟随 App 语言，用户保存后
...
- 自动恢复已接入无人值守预检：Codex 保持前台、存在草稿、目标身份不唯一或另一个恢复正在执行时
  失败关闭，不覆盖用户输入，也不会并发切换任务。
