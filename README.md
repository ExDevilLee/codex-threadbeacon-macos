...
服务重试、最终失败和所选模型容量已满；授权等待仍没有可靠的只读数据源。
...
- 429/503 自动重试显示黄色 `warning`，重试耗尽或所选模型容量已满显示红色 `error`；
...
- `~/.codex/logs_2.sqlite`：以 SQLite read-only 模式只读取当前可见任务的三个白名单
  target，从结构化日志中提取 turn ID、HTTP 429/503、重试次数、明确的模型容量错误和
  最终失败时间。
