...
> 当前阶段（2026-07-23）：核心功能已经基本稳定，进入“真实使用观察 + 问题修复 + 兼容性维护”阶段。
> 暂不主动增加新功能；候选方向继续保留，只有真实用户反馈形成明确问题后才重新评估和排期。
...
- 主任务行使用`活跃数/总数`显示直接 Subagent，例如`2/27`；活跃数来自最近 rollout 的
  已确认运行状态，总数来自父子关系表，并继续使用父子关系补强子任务过滤。
...
- **研究（精确识别等待输入／授权）**：当前只读数据源仍不能可靠区分 `attention`、
  等待用户输入和等待授权。后续仅在出现稳定 hooks、公开事件接口或明确用户反馈时继续评估，验证能否
...
- **已完成并通过真实 UI 复验（任务中断 MVP）**：Rollout 的
  ...
  因此不声称用户一定点击了 STOP；数值型或 malformed `completed_at` 会回退事件顶层时间，不再
  丢弃中断事件；中断不触发提示音或自动恢复。设计见
...
- **已完成（刚完成状态保留时间）**：默认保留 `1 分钟`，可在 Settings 选择 `1～5 分钟`；
  主任务与 Subagent 共用配置，修改后基线刷新，不补播完成提示音。
...
- **暂缓**：显示列和 Subagent 选项配置保留统一入口，但稳定观察期不主动扩展。

## 研究：信息增强与数据适配

### 更多状态与数据

- **已完成（状态标识）**：保留现有红黄绿灯主语义，并以默认开启的色盲安全图标和状态文字
  区分 `needsAction`、`error`、`warning`、`running`、`completed`、`idle` 和 `unknown`，
  不只依赖颜色。
- `Subagent 后续增强`：直接 Subagent 行内展开和可靠的`活跃数/总数`已经完成；独立异常提示、
  更深任务树和任务树 Token 聚合继续保留为后续候选，不在稳定观察期主动开发。可行性边界见
...
- **当前边界（Accessibility）**：AX 树不暴露任务 ID；当前依赖版本敏感的
  ...
  正在执行时停止。
- **MVP 已完成（连续失败熔断）**：按“任务 ID + 异常类型”持久化连续自动恢复次数，各异常可独立
  配置 `1～20` 次，默认 `3` 次，也可关闭熔断持续重试。达到上限后停止控制 Codex App 并记录
  `已熔断`；新的正常 `task_complete` 自动清除该任务全部计数，Settings 支持按任务和异常类型手动
  解除。清空展示日志不会解除熔断。配置迁移、任务与类型隔离、重启持久化和完成时间边界已由自动化
  测试覆盖；真实连续异常、正常完成清零和不限模式仍需持续观察。设计见
  [`自动恢复连续失败熔断`](docs/auto-recovery-circuit-breaker-design.md)。
- **MVP 已完成（自动恢复设置）**：Release 构建移除手工诊断、任务 ID 和测试发送控件，Debug
  ...
  才允许触发。每种规则还可独立配置连续失败熔断次数或选择不限次数。内置提示词跟随 App 语言，
  用户主动保存的提示词和未保存草稿不会被语言切换覆盖。
  设计见
...
- **暂缓**：新增列与显示密度配置不进入当前阶段；如未来根据反馈重新启用，仍应默认可隐藏，
  避免破坏小窗口和外接小屏场景。
...
- **后续候选**：是否增加更多语言由真实用户反馈决定。新增语言需同步补充 String Catalog、
  系统语言映射、排版长度和状态灯/无障碍文案验证，不改变任务标题、Agent 别名、模型名和日志原文。
...
- **已完成（无障碍增强）**：色盲安全状态标识默认开启、可在 Settings 关闭；主任务与 Subagent
  使用颜色、七种独立 SF Symbol 和现有状态文字共同表达状态，保持固定状态槽位和紧凑列宽；
  已有用户主动保存的关闭选择不会被覆盖。
- **已完成（无障碍增强）**：数据源健康入口继续使用盾牌、三角形和八边形配合文字与颜色，
  不只依赖红、黄、绿区分健康程度。
...
1. 持续使用当前版本，优先修复真实出现的状态误判、自动恢复失败、Codex 数据格式变化和 UI 问题。
2. 收集真实用户反馈，记录问题频率、影响范围和现有替代方式；没有明确证据时不启动新功能。
3. 继续验证已交付但依赖真实场景的能力，包括自动恢复与熔断、真实压缩、CLI 长生命周期和副屏恢复。
4. 获得 Developer ID Application 后，再补齐正式签名、公证和登录启动端到端验证。
5. 定期同步 README、故障排查、隐私边界和 ROADMAP；候选功能只在反馈支持时重新排序。
