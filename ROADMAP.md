...
- **待实现（阶段二）**：本机 `codex-cli 0.144.1` 已确认支持
  `codex unarchive <SESSION>`。已归档收藏会话的右键恢复应调用该受支持命令，并处理操作
  确认、失败提示和版本兼容；不直接修改 Codex SQLite。
...
- 通过 `codex unarchive` 恢复归档任务时，如何可靠发现 CLI 路径并处理 App 环境中的
  PATH 差异？
...
1. 已归档收藏通过 `codex unarchive` 恢复的交互与失败路径验证。
