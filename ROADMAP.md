...
- **近期（阶段一）**：主任务右键支持`收藏会话`和`取消收藏`；收藏与现有置顶是两种
  独立语义：置顶只影响当前列表排序，收藏用于形成长期关注集合和筛选范围。
- **近期（阶段一）**：工具栏支持在`全部会话`与`仅显示收藏`之间切换；筛选状态应持久化，
  并提供清晰的空状态和一键返回全部会话入口。
- **近期（阶段一）**：收藏信息只保存稳定任务 ID，不保存标题、正文或会话内容；会话
  rename 后继续沿用 Codex 的最新名称。
- **研究（阶段一）**：已收藏会话即使在 Codex 中归档，也继续出现在收藏列表，并明确
  显示`已归档`标记；归档状态不应被显示成运行中，也不触发完成或异常提示音。
- **研究（阶段一）**：验证已归档会话能否继续可靠读取 rename 后标题、最近状态、Token
  和 Subagent 信息；数据缺失时保留收藏记录，并显示降级状态而不是静默消失。
- **研究（阶段二）**：已归档收藏会话的右键菜单提供`恢复为激活状态`候选操作。该操作
  会改变 Codex 本地数据，必须优先使用官方接口或受支持的命令，并处理操作确认、失败
  提示、并发写入和版本兼容；在没有可靠写入接口前，不直接修改 Codex SQLite。
- **后续**：收藏管理可扩展为批量取消收藏、按项目分组、收藏排序和收藏会话数量提示；
  是否支持“收藏后自动置顶”应作为显式设置，默认不耦合两种行为。
...
- Codex 是否提供受支持的归档恢复接口；如果没有，ThreadBeacon 是否应坚持只读边界？
...
1. 收藏/取消收藏、收藏筛选和已归档收藏只读展示的可行性验证。
2. 状态数据源健康诊断和最小 Settings 收口。
3. 归档恢复官方接口调研，以及扩展状态与 Subagent 后续增强可行性验证。
