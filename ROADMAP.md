...
- `Subagent 数量标记`：**可行性调研已完成**。`thread_spawn_edges` 可以可靠提供直接
  子任务总数，但 `open` / `closed` 不能解释为实时运行状态。建议第一版只显示中性的
  总数标记，并用关系表补强主任务过滤；详见
  [`docs/subagent-count-feasibility.md`](docs/subagent-count-feasibility.md)。
- `Subagent 展开详情`：保留为后续候选；总数标记经过真实使用验证、且获得可靠状态
  数据后，再决定是否展示子代理状态、任务树和 Token 聚合。
