...
- 新增按“任务 ID + 异常类型”独立计数的自动恢复熔断保护。每类异常默认最多连续尝试 `3` 次，
  可在 Settings 中输入 `1...20` 或关闭限制持续尝试；计数跨 App 重启保留，并在任务正常完成后
  自动清零。
- 自动恢复记录新增“已熔断”状态和当前熔断列表。达到上限后停止继续控制 Codex App，用户可按
  任务和异常类型单独解除，不影响其他任务或规则。

### Documentation

- 同步 Subagent `活跃数/总数`、压缩可观测性和当前稳定观察期状态；后续新增功能改为由真实问题
  和用户反馈驱动。
